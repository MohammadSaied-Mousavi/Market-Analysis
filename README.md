...
اسکریپت به‌روزرسانیِ دیتابیس (`updater.py`) و خودِ فایل `database/macro_database.csv` **عمداً در این ریپو نیستن** (توی `.gitignore`). این ریپو کد تحلیل/مدل‌سازی رو نشون می‌ده؛ برای اجرای کامل، به دیتابیس/pipeline خودتون نیاز دارید (یا با من تماس بگیرید).
